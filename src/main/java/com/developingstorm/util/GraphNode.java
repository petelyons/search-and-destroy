...
public class GraphNode<T, S> {
    ...
    /**
     * Hash code based only on content object and state, NOT relatives.
     * Including relatives causes StackOverflowError with cyclic graphs.
     * Graph nodes are considered equal if they wrap the same content object.
     */
    @Override
    public int hashCode() {
        final int prime = 31;
        ...
        // result = prime * result + ((relatives == null) ? 0 : this.relatives.hashCode());
        result = prime * result + ((state == null) ? 0 : this.state.hashCode());
        return result;
    }

    /**
     * Equality based only on content object and state, NOT relatives.
     * Including relatives causes StackOverflowError with cyclic graphs.
     * Graph nodes are considered equal if they wrap the same content object.
     */
    @Override
