...
public class BoardCanvas extends HexCanvas {
    ...
    private Board board;
    //  private int this.width;
    //  private int this.height;
    private Game game;
    private Image[] mapImages;
    private GameIcons icons;
    private UIMode uiMode = UIMode.GAME;
    private volatile Location gameCursor;

    private SaDBoardContext ctx;
    ...
    PathError specificPathAnimationLine;

    private List<AStarState> astarStates;

    public BoardCanvas(Game game, GameIcons icons, SaDBoardContext ctx) {
        super(ctx, game.getBoard());
        this.uiMode = UIMode.GAME; // Initialize before any callbacks
        this.ctx = ctx;
        this.icons = icons;
        this.game = game;
    ...
    public void setCursor(Location loc) {
        if (this.uiMode == null) {
            // Constructor not yet complete - ignore
            return;
        }
        switch (this.uiMode) {
            case GAME: {
    ...
    void showGameCursor() {
        if (this.uiMode == null || this.uiMode != UIMode.GAME) {
            return; // Constructor not complete yet
        }
        if (gameCursor == null) {
            return;
        }
        BoardHex hex = this.board.get(this.gameCursor);
        this.board.setFocus(hex);
    }
    ...
    public void setSelection(Location start, Location end) {
        /*
         * Location oldStart = this.selStart; Location oldEnd = this.selEnd;
         *
         ...
         *
         * this.selStart = Location.get(minx, miny); this.selEnd = Location.get(maxx, maxy);
         * } else { this.selStart = null; this.selEnd = null; }
         */
    }
    ...
    public void setLine(Location start, Location end) {
        ...
        BoardHex bh1 = this.board.get(start);
        BoardHex bh2 = this.board.get(end);
    ...
    private void drawCity(
        ...
        g.setColor(c);
        if (p != null) g.drawPolygon(bh.getHex());
    ...
    private static void drawContinentNumber(
        int num,
        Graphics2D g,
        Point center
    ) {
    ...
    private void drawPathError(Graphics g, PathError pe, Color color) {
        ...
        BoardHex hex1 = this.board.get(pe.start);
        BoardHex hex2 = this.board.get(pe.end);
    ...
    private Image getUnitImage(Unit u) {
        Image[] imgs = this.icons.getImages();
    ...
    private Point getCityCenter(City c) {
        BoardHex bh1 = this.board.get(c.getLocation());
        return bh1.center();
    }
    ...
    public void resetPaths(
        ...
        airPaths = new ArrayList<Sprite>();
        groundPaths = new ArrayList<Sprite>();
        seaPaths = new ArrayList<Sprite>();
    ...
    private void drawUnit(
        ...
        Player p = u.getOwner();
        int size = 14;
        Color c = this.ctx.getPlayerColor(p);
        Color oldColor = g.getColor();
    ...
    private void drawContinentNumbers(Graphics2D g) {
        for (Continent c : this.board.getContinents()) {
            Set<Location> locations = c.getLocations();
            for (Location loc : locations) {
                BoardHex hex = this.board.get(loc.x, loc.y);
                Hex h = hex.getHex();
                Point center = h.getCenter();
                if (this.lens != null && this.lens.isExplored(loc)) {
    ...
    private void drawCities(Graphics2D g) {
        for (City c : this.board.getCities()) {
            ...
            Point center = h.getCenter();
            if (this.lens != null && this.lens.isExplored(loc)) {
                List<Unit> list = this.game.unitsAtLocation(loc);
    ...
    private void drawUnits(Graphics2D g) {
        for (Unit u : this.game.units()) {
            ...
            Point center = h.getCenter();
            if (this.lens != null && this.lens.isExplored(loc)) {
                if (!u.isCarried() && !this.board.isCity(loc)) {
    ...
    protected void drawGameBoard(int z, Graphics2D g) {
        ...
        if (z == 1) {
            ...
            if (this.game != null) {
                Player p = this.game.currentPlayer();
                if (p != null) {
            ...
            if (this.astarStates != null) {
                for (AStarState s : this.astarStates) {
                    AStarPosition pos = s.pos();
                    BoardHex hex = this.board.get(pos.getX(), pos.getY());
                    Point p = hex.center();
            ...
            if (SHOW_PATH_ERRORS) {
                for (PathError pe : this.pathErrors) {
                    drawPathError(g, pe, Color.RED);
                }
            }

            if (this.specificPathAnimationLine != null) {
                drawPathError(g, this.specificPathAnimationLine, Color.ORANGE);
            }
    ...
    public AStarWatcher getAStarWatcher() {
        return new AStarWatcher() {
            public void watch(
                boolean knownError,
                AStarWatcher.AStarRequestState states
            ) {
                if (
                    (SaDFrame.DEBUG_ASTAR || knownError) &&
                    SaDFrame.DEBUG_PATH_TOGGLE
                ) {
                    //Use F5 to toggle the path display
                    if (states != null) {
                        BoardCanvas.this.astarStates = states.states;
                        if (BoardCanvas.this.astarStates != null) {
                            BoardCanvas.this.specificPathAnimationLine =
                                new PathError();
                            BoardCanvas.this.specificPathAnimationLine.start =
                                getLocation(states.start);
                            BoardCanvas.this.specificPathAnimationLine.end =
                                getLocation(states.end);
                        }
                    } else {
                        BoardCanvas.this.astarStates = null;
                        BoardCanvas.this.specificPathAnimationLine = null;
                    }
            ...
            public void displayError(AStarNode start, AStarNode end) {
                ...
                BoardCanvas.this.pathErrors.add(pe);
            }
        };
    }
