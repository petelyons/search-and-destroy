...
public class UnitTurnState {
    ...
    private boolean executeOrder(Unit u, Order alt) {
        ...
        Order executedOrder = (alt != null) ? alt : u.getOrder();
        ...
        if (code == ResponseCode.CANCEL_ORDER) {
            ...
            // Check if a new order was assigned during execution (e.g., auto-sleep)
            if (u.getOrder() == executedOrder) {
                u.turn().clearOrderAndCompleteTurn();
            } else {
                // New order was assigned, just complete the turn
                u.turn().completeTurn();
            }
        } else if (code == ResponseCode.ORDER_COMPLETE) {
            // Check if a new order was assigned during execution (e.g., auto-sleep)
            if (u.getOrder() != executedOrder) {
                // New order was assigned, don't clear it
                Log.debug(
                    u,
                    "New order assigned during execution, preserving it"
                );
            } else {
                u.clearOrders();
            }
