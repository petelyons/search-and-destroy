    ...
    public OrderResponse executeInternal() {
        ...
        do {
            ...
            if (this.unit.life().mustLand() && !this.unit.hasLanded()) {
                ...
                ResponseCode headHomeCode = response.getCode();
                if (headHomeCode == ResponseCode.ORDER_COMPLETE) {
                ...
                if (headHomeCode == ResponseCode.CANCEL_ORDER) {
                    Log.warn(
                        this.unit,
                        "Cannot reach home base - path blocked!"
                    );
                    return new OrderResponse(
                        ResponseCode.TURN_COMPLETE,
                        this,
                ...
                if (
                    headHomeCode == ResponseCode.TURN_COMPLETE ||
                    headHomeCode == ResponseCode.ORDER_AND_TURN_COMPLETE
                ) {
                    return new OrderResponse(
                        ResponseCode.TURN_COMPLETE,
                        this,
            ...
            if (frontierLocations.isEmpty()) {
                ...
                if (this.unit.getTravel() == Travel.AIR) {
                    if (!this.unit.hasLanded()) {
                        ...
                        City currentCity = this.game.cityAtLocation(
                            this.unit.getLocation()
                        );
                        City hop = this.unit.getOwner().findHopCity(
                            currentCity,
                            this.unit.getMaxTravel()
                        );
                        if (hop != null) {
                            Order altMove = new Move(
                                this.game,
                                this.unit,
                                hop.getLocation()
                            );
            ...
            if (ul.distance(loc) > 1) {
                ...
                if (path != null && !path.isEmpty()) {
                    ...
                    Log.debug(
                        this.unit,
                        "Blocked from exploring choosen location! " + loc
                    );
            ...
            if (
                resp == ResponseCode.DIED || resp == ResponseCode.TURN_COMPLETE
            ) {
                return new OrderResponse(resp, this, null);
            }

            if (
                resp == ResponseCode.YIELD_PASS &&
                blockedLocations.contains(dest)
            ) {
                return new OrderResponse(
                    ResponseCode.TURN_COMPLETE,
                    this,
